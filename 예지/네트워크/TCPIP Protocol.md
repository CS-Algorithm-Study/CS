...
<img width="434" alt="OSI" src="https://github.com/CS-Algorithm-Study/CS/assets/48826098/1e01dacc-3c9a-4f5d-873e-71474bf1caa4">

## ⚙️ 물리계층

- 주로 전기적, 기계적, 기능적 특성을 이용해 통신 케이블로 데이터 전송하는 물리적 장비
- 단지 데이터 전기적인 신호로 변환해서 주고받는 기능
- 데이터 단위는 Bit
- 장비
    - 통신 케이블
    - 리피터
...
- 프레임에 물리적 주소(MAC주소)를 부여하고 오류 검출, 재전송, 흐름제어 수행
- 데이터 단위 Frame
- 장비
    - 브리지
    - 스위치
...
- 라우터를 통해 경로를 선택하고 주소를 정하고 경로에 따라 패킷 전달
- 데이터 단위 Packet
- 장비
    - 라우터
...
- TCP/IP 세션 체결, 포트번호 기반으로 통신 세션 구성
...
- HTTP, FTP, SMTP, TELNET 같은 프로토콜 존재

→ 근데 네트워크 강의 들었을 때 OSI가 더 최근에 나왔지만 잘 안쓴다고..? 들었던거 같다!
